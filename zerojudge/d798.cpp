...
struct seg1D {
    int n;
    seg1D *lc, *rc;
    seg1D(int val = 0) : n(val), lc(nullptr), rc(nullptr) {}
};
...
int query1D(seg1D *o, int l, int r, int L, int R) {
    ...
    int m = (l + r) >> 1;
    return max(query1D(o->lc, l, m, L, R), query1D(o->rc, m + 1, r, L, R));
}
// 2D segment tree -----------
struct seg2D {
    seg1D *S;
    seg2D *lc, *rc;
    seg2D(int val = 0) : S(nullptr), lc(nullptr), rc(nullptr) {}
};
...
int query2D(seg2D *&o, int l = 1, int r = N) {
    ...
    if (qy <= l && r <= qY) {
        ...
        int m = (l + r) >> 1;
        return max(query2D(o->lc, l, m), query2D(o->rc, m + 1, r));
    }
}
