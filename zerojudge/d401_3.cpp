...
int main() {
    ...
    while (gets(A), gets(B)) {
        ...
        // Password should contain at least one upper-case alphabetical
        // character.
        memset(num, 0, sizeof(num));
        for (int i = 0; i < strlen(A); i++) num[(int)A[i] + 200]++;
        bool ok = 0;
        for (int i = 'A' + 200; i <= 'Z' + 200; i++)
            if (num[i] > 0) {
                ok = 1;
                break;
            }
        ...
        // Password should contain at least one lower-case alphabetical
        // character.
        ok = 0;
        for (int i = 'a' + 200; i <= 'z' + 200; i++)
            if (num[i] > 0) {
        ...
        ok = 0;
        for (int i = 0, l = strlen(A); i < l / 2; i++)
            if (A[i] != A[l - 1 - i]) {
                ok = 1;
                break;
            }
